...
tf.app.flags.DEFINE_string('train_dir', '/tmp/try1',
...
def value_to_class(v):
    ...
    if df > foreground_threshold:
        return [1, 0] #              ***** CHANGE THIS???? alreday did
    else:
        return [0, 1]
...
def main(argv=None):  # pylint: disable=unused-argument
    ...
    conv1_weights = tf.Variable(
        tf.truncated_normal([64, 64, NUM_CHANNELS, 32],  # 5x5 filter, depth 32.
                            stddev=0.1,
                            seed=SEED))
    conv1_biases = tf.Variable(tf.zeros([32]))
    conv2_weights = tf.Variable(
        tf.truncated_normal([50, 50, 32, 64],
                            stddev=0.1,
                            seed=SEED))
    ...
    with tf.Session() as s:
            ...
            for iepoch in range(num_epochs):
                ...
                for step in range (int(train_size / BATCH_SIZE)):
                    ...
                    if step % RECORDING_STEP == 0:
                        ...
                        print ('Epoch %.2f' % (iepoch + float(step) * BATCH_SIZE / train_size))
        ...
        """print("Computing F1 on other images")
        idx = numpy.random.permutation(numpy.arange(100))
        #test_labels = extract_labels(train_labels_filename, 100)

        for i in range(20):
            imageid = "satImage_%.3d" % idx[i]
            image_filename = train_data_filename + imageid + ".png"
            img = mpimg.imread(image_filename)
            img_pred = get_prediction(img)
            
            image_filename = train_labels_filename + imageid + ".png"
            gt_img = mpimg.imread(image_filename)
            img_lab = gt_img.round().astype(numpy.uint8)

            print(f1_score(img_lab, img_pred))"""
